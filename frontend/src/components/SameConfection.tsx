...
function SameConfection(){
    const [modal1,setModal1]=useState(false)
    const [select,setSelect]=useState('fresh')
    ...
    const submitHandler2=()=>{
      axios.post('http://localhost:8000/kitchen/updatedetails',{
        ...
        headers:{"Content-type":"application/json"}}).then(Res=>{setModal1(!modal1)}).catch(Err=>{console.log(Err)})  
    ...
    const changeHandler=(e:any)=>{
      ...
      useEffect(()=>{
      axios.post('http://localhost:8000/kitchen/sameconfection',{confection:select,headers:{"Content-type":"application/json"}})
  ...
  //  const admin = useSelector((state) => state.admin);
    ...
    return (
     ...
     <div>
        <div style={{textAlign:'center'}}><h2>INGREDIENTS IN SAME CATEGORY</h2></div>
         <Container>   
<Paper className={classes.pageContent}>
 <Toolbar className={classes.toolbarContent}>
   ...
   <Controls.Button
     text="Confection"
     variant="outlined" />
     <Controls.Select name="category"  value={select}  onChange={changeHandler} error={null}
      options={[{id:"1",title:"fresh"},{id:"2",title:"canned"},{id:"3",title:"frozen"},{id:"4",title:"cured"}]}
     />
